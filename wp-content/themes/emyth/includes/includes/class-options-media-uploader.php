...
class Options_Framework_Media_Uploader {
	...
	static function optionsframework_uploader( $_id, $_value, $_desc = '', $_name = '' ) {
		...
		$output .= '<input id="' . $id . '" class="upload' . $class . '" type="text" name="'.$name.'" value="' . $value . '" placeholder="' . __('No hay archivo', 'emyth') .'" />' . "\n";
		if ( function_exists( 'wp_enqueue_media' ) ) {
			if ( ( $value == '' ) ) {
				$output .= '<input id="upload-' . $id . '" class="upload-button button" type="button" value="' . __( 'Subir', 'emyth' ) . '" />' . "\n";
			} else {
				$output .= '<input id="remove-' . $id . '" class="remove-file button" type="button" value="' . __( 'Remover', 'emyth' ) . '" />' . "\n";
			}
		} else {
			$output .= '<p><i>' . __( 'Actualiza tu versión de WordPress para un completo soporte multi-media.', 'emyth' ) . '</i></p>';
		}
		...
		if ( $value != '' ) {
			$remove = '<a class="remove-image">Remover</a>';
			$image = preg_match( '/(^.*\.jpg|jpeg|png|gif|ico*)/i', $value );
			if ( $image ) {
				$output .= '<img src="' . $value . '" alt="imagen" />' . $remove;
			} else {
				$parts = explode( "/", $value );
	...
	function optionsframework_media_scripts( $hook ) {
		...
		wp_localize_script( 'of-media-uploader', 'optionsframework_l10n', array(
			'upload' => __( 'Subir', 'emyth' ),
			'remove' => __( 'Remover', 'emyth' )
		) );
	}
}